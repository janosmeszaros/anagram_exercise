...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    ...
    val result: Seq[Occurrences] = for {
      len <- 0 to occurrences.length
      combin <- toSub combinations len
    } yield combin

    result.toList.filter(occ => !occ.groupBy(f => f._1).exists(t => t._2.length > 1))
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val toSubtract = y.toMap
    x.map((t: (Char, Int)) =>
      toSubtract get t._1 match {
        case None => (t._1, t._2)
        case Some(number) => (t._1, t._2 - number)
      })
      .filter((t: (Char, Int)) => t._2 > 0)
  }
